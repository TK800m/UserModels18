...
#Â· Send any configuration commands to the EyeLink tracker to prepare it for the experiment
#Â· Get an EDF file name, and open an EDF data file (stored on the eye tracker)
#Â· Record one or more blocks of trials. Each block typically begins with tracker setup (camera setup
#and calibration), and then several trials are run.
#Â· Close the EDF data file. If desired, copy it via the link to the local computer.
#Â· Close the window, and the link connection to the eye tracker.
##---------------------------------------------------------------------------------------------------------------------------------------------------------##
...
def initiate():
    ...
    word_speed = -2.5 # pixels
    basis_frame_time = 1.0 / FPS

    return currText, word_speed, basis_frame_time, data, txtObj, B1, B2, B3, B4, A1, A2, A3, A4
...
def calcWIV(baseline, threshold):
	pupilsize  = pl.getEventDataFlags(time.currentTime())
	pcps = (pupilsize-baseline)/baseline + 1000
	average = 0
	liveCount = 0

	while time.currentTime < 10000:
...
def game_loop(currText, word_speed, txtObj, B1, B2, B3, B4, A1, A2, A3, A4):
    textCounter = 0
    ...
    while True:
        ...
        if question_screen == False:
            ...
            if (count > 20):
                objectTrackers = objectTrackers+ word_speed
                count = 0
...
currText, word_speed, basis_frame_time, data, txtObj, B1, B2, B3, B4, A1, A2, A3, A4 = initiate()
info = 'press the number corresponding to the correct answer..'

game_loop(currText, word_speed, txtObj, B1, B2, B3, B4, A1, A2, A3, A4)

'''
if(pl.getEYELINK().isConnected() and not pl.getEYELINK().breakPressed()):
        game_loop()
